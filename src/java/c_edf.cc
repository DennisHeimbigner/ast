...
EDFGenerator::EDFGenerator(const EDFDescriptor* descriptor,
...
// Header stuff.
void EDFGenerator::GenerateMainHFile(io::Printer* printer)
{
  GenerateVfuncs(printer);
  GenerateInitMacros(printer);
  GenerateCallersDeclarations(printer);
}
void EDFGenerator::GenerateVfuncs(io::Printer* printer)
{
  printer->Print(vars_,
		 "typedef struct _$cname$_EDF $cname$_EDF;\n"
		 "struct _$cname$_EDF\n"
		 "{\n"
		 "  ProtobufCEDF base;\n");
  for (int i = 0; i < descriptor_->method_count(); i++) {
    ...
    printer->Print(vars_,
                   "  void (*$method$)($cname$_EDF *service,\n"
  ...
  printer->Print(vars_,
		 "};\n");
  printer->Print(vars_,
		 "typedef void (*$cname$_EDFDestroy)($cname$_EDF *);\n"
		 "void $lcfullname$__init ($cname$_EDF *service,\n"
		 "     $lcfullpadd$        $cname$_EDFDestroy destroy);\n");
}
void EDFGenerator::GenerateInitMacros(io::Printer* printer)
{
  printer->Print(vars_,
...
void EDFGenerator::GenerateCallersDeclarations(io::Printer* printer)
{
  for (int i = 0; i < descriptor_->method_count(); i++) {
    const MethodDescriptor *method = descriptor_->method(i);
    string lcname = CamelToLower(method->name());
    ...
    printer->Print(vars_,
                   "void $lcfullname$__$method$(ProtobufCEDF *service,\n"
...
void EDFGenerator::GenerateDescriptorDeclarations(io::Printer* printer)
{
  printer->Print(vars_, "extern const ProtobufCEDFDescriptor $lcfullname$__descriptor;\n");
}


// Source file stuff.
void EDFGenerator::GenerateCFile(io::Printer* printer)
{
  GenerateEDFDescriptor(printer);
  GenerateCallersImplementations(printer);
  GenerateInit(printer);
}
void EDFGenerator::GenerateInit(io::Printer* printer)
{
  printer->Print(vars_,
		 "void $lcfullname$__init ($cname$_EDF *service,\n"
		 "     $lcfullpadd$        $cname$_EDFDestroy destroy)\n"
...
void EDFGenerator::GenerateEDFDescriptor(io::Printer* printer)
{
  ...
  printer->Print(vars_, "const ProtobufCEDFDescriptor $lcfullname$__descriptor =\n"
                       "{\n"
...
void EDFGenerator::GenerateCallersImplementations(io::Printer* printer)
{
  for (int i = 0; i < descriptor_->method_count(); i++) {
    const MethodDescriptor *method = descriptor_->method(i);
    string lcname = CamelToLower(method->name());
