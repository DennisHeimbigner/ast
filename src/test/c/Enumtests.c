...
Enumtest_write(ast_runtime* rt, Enumtest* enumtest_v)
{
    ...
    {
        status = ast_write_primitive_packed(rt,ast_enum,4,&enumtest_v->ppenum);
    }
...
Enumtest_read(ast_runtime* rt, Enumtest** enumtest_vp)
{
    ...
    while(status == AST_NOERR) {
        ...
        case 4: {
            status = ast_read_primitive_packed(rt,ast_enum,4,&enumtest_v->ppenum);
            if(status != AST_NOERR) {ACATCH(status); goto done;}
            } break;
        default:
...
Enumtest_get_size(ast_runtime* rt, Enumtest* enumtest_v)
{
    ...
    }
    {
        ...
        for(i=0;i<enumtest_v->ppenum.count;i++) {
            fieldsize += ast_get_tagsize(rt,ast_enum,4);
